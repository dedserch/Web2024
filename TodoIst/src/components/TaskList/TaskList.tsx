...
export class TaskList extends Component<TaskListProps, TaskListState> {
  ...
  handleScroll = debounce(() => {
    ...
    if (taskListElement) {
      const bottomReached =
        taskListElement.scrollHeight - taskListElement.scrollTop <=
        taskListElement.clientHeight + 10

      if (bottomReached && visibleTasksCount < tasks.length) {
        this.setState((prevState) => ({
          visibleTasksCount: prevState.visibleTasksCount + 4,
        }))
      }
    }
  }, 500)
  ...
  render() {
    ...
    return (
      <div
        id="task-list"
        className="overflow-y-auto h-64 overflow-hidden scrollbar-thin scrollbar-thumb-gray-400 scrollbar-track-gray-100 size-full"
      >
